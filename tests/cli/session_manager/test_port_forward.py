...
def test_dry_run_localhost(snapshot: Snapshot) -> None:
    # Arrange
    ...
    with (
        mock.patch(
            ...
            return_value=["session-manager-plugin"],
        ),
...
def test_dry_run_non_localhost(snapshot: Snapshot) -> None:
    # Arrange
    ec2 = boto3.client("ec2")
    response = ec2.run_instances(
        ImageId="ami-12345678",
    ...
    with (
        mock.patch("aws_annoying.cli.session_manager.port_forward.get_free_port", return_value=54321),
        mock.patch(
            "aws_annoying.session_manager.SessionManager.build_command",
            return_value=["session-manager-plugin"],
        ),
...
def test_port_forward_non_localhost(snapshot: Snapshot) -> None:
    ...
    with (
        mock.patch("aws_annoying.cli.session_manager.port_forward.get_free_port", return_value=54321),
        mock.patch("aws_annoying.cli.session_manager.port_forward.TCPProxy") as mock_proxy_cls,
        mock.patch(
            "aws_annoying.session_manager.SessionManager.build_command",
            return_value=["session-manager-plugin", "arg1"],
        ) as mock_build_cmd,
