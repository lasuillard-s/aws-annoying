...
It is recommended to use [uvx](https://docs.astral.sh/uv/concepts/tools/) (alias for `uv tool run`) to run `aws-annoying` CLI (you can also use `pipx run` if you prefer):
...
To use browser user scripts to improve your AWS Console experience, download the scripts from the [console](./console) directory and install them in your browser.

Note that these scripts are provided "as is" and may not work in all cases.

### 🐳 Dev Container Features

You can use the Dev Container Features provided in [devcontainer-features](./devcontainer-features/) directory to help set up development containers. For example, add it in your `devcontainer.json` file:
